...
class ProductController extends Controller
{
    ...
    public function productData(Request $request)
    {
        ...
        if(empty($request->input('search.value'))) {
            ...
            $totalFiltered = Product::join('categories', 'products.category_id', '=', 'categories.code')
                            ->leftjoin('brands', 'products.brand_id', '=', 'brands.code')
                            ->where([
                                ['products.name','LIKE',"%{$search}%"],
                                ['products.is_active', true]
                            ])
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                $nestedData['cost'] = number_format($product->cost, 0, ',', '.');

                if(config('currency_position') == 'prefix')
                    $nestedData['stock_worth'] = number_format(($product->qty * $product->price), 0, ',', '.').' / '.number_format(($product->qty * $product->cost), 0, ',', '.');
                else
                    $nestedData['stock_worth'] = number_format(($product->qty * $product->price), 0, ',', '.').' / '.number_format(($product->qty * $product->cost), 0, ',', '.');
    ...
    public function productWarehouseData($id)
    {
        ...
        $product_warehouse = [$warehouse, $qty, $batch, $expired_date, $imei_number, $price];
    ...
    public function generateCode($nmProduct, $codeBrand, $codeCategories)
    {
        ...
        $validateName = Product::select('code')
                            ->where('name',$nmProduct)
                            ->orderby('code', 'desc')
                            ->first();

        if ($validateName) {
            $codeProduct = $validateName->code;
        } else {
            $findCode = Product::select('code')
                            ->where('code', 'like', '%'.$inptCode.'%')
                            ->orderby('code', 'desc')
                            ->first();

            if ($findCode != null) {
                $serialNumber = substr($findCode->code,16,4)+1;
                $length = strlen($serialNumber);

                if ($length == 1) {
                    $codeProduct = $inptCode.date('mY')."000".$serialNumber;
                } elseif ($length == 2) {
